...
import { ISchema, resolveRefs } from '@shellagent/form-engine';
...
function addAttributesToPrimitive(schema: JsonSchema7): ISchema {
  ...
  if (newSchema.type && /[A-Z]/.test(newSchema.type)) {
    return {
      ...newSchema,
      type: 'string',
...
const getSchemaByWidget = ({
  ...
  if (!isEmpty(input_schema)) {
    ...
    if (!multi_input_schema) {
      ...
      schema.title = 'Input';
      properties.input = addCustomUIProperties(schema);
    } else {
      ...
      if (properties.input.properties?.basic) {
        set(properties.input.properties?.basic, 'title', 'Input');
      }
      if (properties.input.properties?.advanced) {
        set(properties.input.properties?.advanced, 'title', 'Input');
      }
    }
  }
