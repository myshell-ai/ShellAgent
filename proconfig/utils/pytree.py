"""
Copy from https://github.com/pytorch/pytorch/blob/main/torch/utils/_pytree.py
...
class TreeSpec:
    ...
    def __post_init__(self) -> None:
        self.num_nodes = 1 + \
            sum(spec.num_nodes for spec in self.children_specs)
            ...
            if both_standard_dict:  # dictionary types are compatible with each other
                ...
                if (
                    ...
                    raise ValueError(
                        ...
                        f"expected {self.context!r}, but got {context!r}.",
                    )
...
def _tree_flatten_helper(tree: PyTree, leaves: List[Any]) -> TreeSpec:
    ...
    children_specs = [_tree_flatten_helper(
        child, leaves) for child in child_pytrees]
